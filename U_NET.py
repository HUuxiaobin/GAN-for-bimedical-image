...
#Codes are written by Mr. Hongwei Li (h.l.li@dundee.ac.uk) and Mr. Gongfa Jiang (jianggfa@mail2.sysu.edu.cn). They are PhD students in Sun Yat-sen University and Technical University of Munich.
#Please cite our paper titled 'Fully Convolutional Networks Ensembles for White Matter Hyperintensities Segmentation in MR Images' if you found it was useful to your research.
...
from keras.layers.merge import concatenate #####concatenate is to tie up the vector
from keras.optimizers import Adam
from keras.callbacks import ModelCheckpoint ###after every epoch save as filepath
from keras import backend as K ##call for the back enginee for tf or theano
from keras.preprocessing.image import apply_transform, transform_matrix_offset_center
import warnings
K.set_image_data_format('channels_last') ##define the dimension of image, (rows,cols,channels)

smooth=1.
print(os.getcwd())
##dice coefficent index
def dice_coef_for_training(y_true, y_pred):
    y_true_f = K.flatten(y_true) ##change matrix into one vector default is raw
    y_pred_f = K.flatten(y_pred)
    intersection = K.sum(y_true_f * y_pred_f)
    return (2. * intersection + smooth) / (K.sum(y_true_f) + K.sum(y_pred_f) + smooth)
#smooth is to prevent the zero problem
...
def get_unet(img_shape = None, first5=True):
        ...
        model.compile(optimizer=Adam(lr=(2e-4)), loss=dice_coef_loss)###important model
...
def train_leave_one_out(images, masks, patient, full=True, first5=True, aug=False, verbose=False):
    ###(760,1520,2280),then 0-20 slice=38;20-40,slice=38;40-60,slice=63)
    if full:
        if patient < 40:
...
def main():
    ...
    print(images.shape)  #(2780, 200, 200, 2)
    print(masks.shape)   #(2780, 200, 200, 1)         
    patient_num = 60
